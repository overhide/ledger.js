...
![](ignore/what-demo.png)
*Figure 1: What's in this repository.*

## [1] Before You Run the Demo App

When you start the [demo](login.html) take note of the BOTTOM-RIGHT pane which displays some information on the UI.

To make sense of the [demo](login.html); do read through the introductory material below, which ends with a sample play-by-play run-through of a [demo](login.html) session in section 4.
...
These API methods are used in the [*Service Code*](service.html) as part of ledger-based authorization: read more about this authorization flow in the [why/justification](why/why.html) piece.
...
This [demo](login.html) interacts with the *Rinkeby* *Ethereum* testnet and the [overhide-ledger test environment](https://test.ohledger.com).  This demo is for demonstration purposes.  

Use a [Rinkeby](https://faucet.rinkeby.io/) faucet to get "test" Ether for playing around with Ethereum in the [demo](login.html).

Use [Stripe's "test" credit cards](https://stripe.com/docs/testing#cards) to play around with dollar transactions in the [demo](login.html).
...
This is a play-by-play run-through of the [Demo](login.html).

Make sure you have a fresh [Demo](login.html) session open, then read-on.

**Make sure you're looking at the [Demo](login.html) in widescreen mode**.
...
If you get-ahead of this run-through and click on any logs (in the TOP-RIGHT pane) before you finish reading the orientation text (in the BOTTOM-RIGHT pane) the orientation text will disappear.  Simply [re-open the demo](login.html) or refresh your browser.
...
```
       [...] (login.html) :: global settings: paymentSchedule : {
                               "eth-web3": {
...
Once clicked the BOTTOM-RIGHT pane jumps to the the source code in [login.html](TODO_SRC) that printed out the above logs.

Note the *log(..)* call jumped-to: `log('global settings: paymentSchedule', paymentSchedule);`.  It is right at the start of [login.html](TODO_SRC) in one of the first `<script>` tags.

What's logged here is our *payment schedule* configuration which resides in the [config.js](TODO_SRC_CONFIG) file (in this repo, part of demo app).

This *payment schedule* sits in its own file as we use it in both the front-end [login.html](TODO_SRC) as well as the [service.html](TODO_SRC_SERVICE) files.
...
```
[...] (login.html) :: UI >> has USD selected
```

It orignates from the `ui.onFiatSelected = () => {..}` callback in [login.html](TODO_SRC).  Click the log to see the code.

The `ui` namespace containing *onFiatSelected(..)* is declared at the top of [ignore/login.ui.js](TODO_SRC_LOGIN_UI_JS) in the global `ui = { .. }` object.  The rest of the UI callbacks are also declared here.  

Although the callbacks are declared in [ignore/login.ui.js](TODO_SRC_LOGIN_UI_JS), they are all assigned towards the end of [login.html](TODO_SRC).
...
[login.html](TODO_SRC) assigns functionality to these callbacks, but the rest of the UI implementation is tucked away inside of [ignore/login.ui.js](TODO_SRC_LOGIN_UI_JS) along side the template in [ignore/login.ui.html](TODO_SRC_LOGIN_UI_HTML).  Files inside the [ignore](TODO_SRC_IGNORE) folder are implementation details of the demo--such as styling, utilities for for the code and logging panes, and lower level UI state changing code.  They're in the [ignore](TODO_SRC_IGNORE) folder to avoid cluttering [login.html](TODO_SRC_LOGIN).

**( o_o) Quick recap of files in this repo.**

Reading about the [ignore](TODO_SRC_IGNORE) folder above you might be wondering what all the files might be.  Below is a quick summary:
...
**(¬-_-)¬ Click the `[...](login.html) :: UI >> onGenerate` log.**
...
**(¬-_-)¬ Click the `[...](login.html) :: OH >> onCredentialsUpdate :: wallet callback called--address updated: {..}` log.**
...
Now that we see the panes, we also see the `[...](login.html) :: UI >> onGoCommon` line in the logging pane.

**(¬-_-)¬ Click the `[...](login.html) :: UI >> onGoCommon` log.**
...
A core concept of the [ledger-based authorization](why/why.html)--which we're going through--is that all tiers of authorization require a transaction on the selected ledger: any thransaction, even if it's a transaction to yourself or someone other than the service provider you're signing into.  Hence free transactions still go through [oh$.createTransaction](https://test.ohledger.com/demo/ignore/oh$-rendered-docs/index.html#createtransaction) should they fail the [oh$.isOnLedger](https://test.ohledger.com/demo/ignore/oh$-rendered-docs/index.html#isonledger) check.

Existence of a transaction will once again be verified in the [*service code*](service.html) with a call to the selected ledger's *remuneration API*'s `/is-signature-valid`.
...
You are now on a new page.  You're in the *service*.  The logs and code you see will now be from [service.html](TODO_SRC_SERVICE).
...
These logs show the same *payment schedule* as we saw before in [login.html](TODO_SRC); also loaded from [config.js](TODO_SRC_CONFIG_JS).  
...
Whereby [login.html](TODO_SRC) needs the *payment schedule* to aid the user in paying, [service.html](TODO_SRC_SERVICE) needs it to validate payment before allowing asked-for authorizations.
...
Since there are only two methods in the remuneration API--and these methods are identical for all ledgers--the [service.html](TODO_SRC_SERVICE) is streamlined by simple parametrization of the same code flow.

**(¬-_-)¬ Go ahead, click "Play Level".**

You earned it!

**(¬-_-)¬ Click on any *login.html* link to go back to the [login.html](TODO_SRC) page.**
