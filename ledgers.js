 ...
 * 
 * The library is usually used by loading straight into your HTML and accessing the `oh$` property from the browser's
 * `window` object:
 * 
 * ```
...
(function() {
  ...
  async function setCredentials(imparterTag, credentials) {
    switch (imparterTag) {
      case OHLEDGER_IMPARTER_TAG:
        ...
        try {
          if (!(web3.eth.accounts.recover(web3.eth.accounts.sign('test message', credentials.secret)).toLowerCase() == credentials.address.toLowerCase())) {
  ...
  async function generateCredentials(imparterTag, options) {
    switch (imparterTag) {
      case OHLEDGER_IMPARTER_TAG:
        let res = window.web3.eth.accounts.create();
